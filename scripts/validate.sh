#!/bin/bash
CODEDEPLOY_APPLICATION=chris-github-new
CODEDEPLOY_DEPLOYMENTGROUP=chris-github-2
BUCKET=testncode
...
if [ "$HTTP_CODE" -eq "200" ]
  ...
  echo $KEY
  ROLLBACK_DEPLOYMENT=`aws deploy create-deployment --application-name ${CODEDEPLOY_APPLICATION} --deployment-config-nam CodeDeployDefault.OneAtATime --deployment-group-name ${CODEDEPLOY_DEPLOYMENTGROUP} --s3-location bucket=${BUCKET},bundleType=zip,key=${KEY} --region us-east-1 --output text`
