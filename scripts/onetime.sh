...
echo $KEY
ROLLBACKDEPLOYMENT=`aws deploy create-deployment --application-name ${CODEDEPLOY_APPLICATION} --deployment-config-name CodeDeployDefault.OneAtATime  --deployment-group-name ${CODEDEPLOY_DEPLOYMENTGROUP} --s3-location bucket=${BUCKET},bundleType=zip,key=${CODEDEPLOY_APPLICATION}/${CODEDEPLOY_DEPLOYMENTGROUP}/${ZIPFILE} --region us-east-1 --output text`
